...
namespace Vikin\WeChatPay\Resource\Lib;

use Illuminate\Support\Facades\App;
use Vikin\WeChatPay\Resource\Lib\WxPayException;
use Vikin\WeChatPay\Resource\Lib\WxPayDataBase;
...
//            throw App::make('Vikin\WeChatPay\Resource\Lib\WxPayException', ['message'=>'业务结果，缺少必填参数result_code！']);
            throw new \Vikin\WeChatPay\Resource\Lib\WxPayException('业务结果，缺少必填参数result_code！');
        }
    ...
    {
        //如果不需要上报数据
        if (config('WeChatConfig.REPORT_LEVENL') == 0) {
            return;
        }
        //如果仅失败上报
        if (config('WeChatConfig.REPORT_LEVENL') == 1 &&
    ...
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        if ($useCert == true) {
            ...
            curl_setopt($ch, CURLOPT_SSLCERT, config('WeChatConfig.SSLCERT_PATH'));
            curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
            curl_setopt($ch, CURLOPT_SSLKEY, config('WeChatConfig.SSLKEY_PATH'));
        }
        //post提交方式
